...
|---|---|---|---|---|
| Cílený test veřejné hranice a normalizace | `node --test --test-isolation=none tests/generate-docs.test.js` | Žádné | Konzolový TAP výstup | 6 testů včetně zápisu .NET, českých tokenů a vypnutého příspěvkového bloku; běžně pod 1 sekundu |
...
| Automatizované testy | `npm test` | Lokální Git a obnovené npm závislosti | Konzolový TAP výstup | Všechny soubory v `tests/`; 13 scénářů |
