...
| Neúplný, nečitelný nebo prostředím ovlivněný changelog | Skutečný `git-cliff` nad dočasnou historií s tagem, conventional, breaking i legacy commitem ve dvou časových prostředích | Chrání úplnost historie, identitu zdrojového commitu, stabilní kotvy, reader-first pořadí, sbalené technické záznamy, breaking marker a determinismus výstupu |
...
Changelogový test vytváří izolovaný Git repozitář v dočasném adresáři a spouští stejný uzamčený binární příkaz jako projektový build bez změny pracovního stromu; následně kontroluje i vazbu hlavičky na jeho skutečný `HEAD`.
