...
DocFX soubor `global.json` nevyžaduje; [architektura](../overview.md) odděluje lokální výběr SDK od řízené instalace v CI.
...
Všechny varianty lze vrátit změnou konfigurace; přijatá varianta odpovídá explicitní preferenci vlastníka.

## Rozhodnutí

Odstraňujeme `global.json`; lokální výběr SDK se řídí standardním chováním .NET a [politikou závislostí](../../development/dependencies.md#výběr-net-sdk).
...
Tímto se nahrazuje pouze přesné připnutí SDK z ADR-0002; veřejná hranice, přesné verze ostatních nástrojů a publikační model zůstávají platné.

## Důsledky a rizika

Běžná aktualizace nainstalovaného SDK nevyžaduje commit.

Lokální prostředí a CI mohou použít odlišné SDK; pro diagnostiku je nutná skutečná verze z logu a případná reprodukce v izolovaném prostředí.
...
Vlastníkem reakce na regresi je engineering; přechod CI na další řadu zůstává řízenou aktualizací.
...
Projekt negeneruje dočasný `global.json`; kompatibilitu lokálního SDK s DocFX prokazuje skutečná obnova a sestavení.
...
Rozhodnutí je přijaté; další změna rozsahu SDK používá [životní cyklus ADR](README.md).
