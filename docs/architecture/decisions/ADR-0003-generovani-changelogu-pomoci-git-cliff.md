...
Projekt uchovával dvě JavaScript konfigurace pro `conventional-changelog`, ale neměl odpovídající npm závislost, lockfile ani projektový příkaz, který by changelog reprodukovatelně generoval.

Verzovaný `changelog.md` se proto udržoval ručně a obsahoval i záznamy starší než obnovená Git historie.

Cílem je generovat aktivní veřejný changelog při každém buildu stejným mechanismem jako v souvisejícím dokumentačním projektu, zachovat čitelné české kategorie a nezamlčet nekonvenční ani breaking commity.

Rozhodnutí podporuje reprodukovatelnost `QLT-002` v [`../../product/requirements.md`](../../product/requirements.md) a build hranici popsanou v [`../overview.md`](../overview.md).
...
|---|---|---|---|---|
| Současný tok není reprodukovatelný | Projektový baseline: `package.json`, `changelog-config*.js` a `npm run verify` | 2026-08-28 | Build prošel, aniž by changelog generoval, a npm závislost ani lockfile neexistovaly | Historické ruční položky nelze po resetu odvodit z Gitu |
...
Aktivní `changelog.md` je ignorovaný build vstup a ručně dochované záznamy před resetem historie zůstávají ve zmrazeném `changelog-archive.md`.
...
Poté build začne generovat ignorovaný `changelog.md` před DocFX, CI obnoví npm balíčky a úplnou historii a překonané JavaScript konfigurace se odstraní.

Historické ruční záznamy zůstávají dostupné v neměnném archivu a nové záznamy se do něj nepřidávají.
