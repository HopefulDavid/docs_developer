...
| Statický web | Čistý build `_site/` a publikovaná větev `gh-pages` | Delivery | Artefakt check porovnává manifest a výstup s veřejnou hranicí | Lokální `_site/` je odstranitelný; vzdálenou historii řídí publish akce | Neobsahuje datové migrace a lze jej znovu sestavit ze zdroje |
...
| GitHub Actions publish | Jeden job na Ubuntu s `contents: write` | Dočasný checkout, `_site/` a deployment commit | GitHub runner, NuGet a GitHub API | Concurrency skupina nepovolí souběžné publikování | GitHub Actions log a commit `gh-pages` |
...
| `ARCH-RISK-004` | Skutečnost: živá branch protection, Pages settings a aktuální vzdálený `gh-pages` nebyly lokálně ověřené | Lokální dokumentace nemusí zachytit ruční změnu platformy | Po změně workflow ověřit první vzdálený quality i publish běh a nastavení platformy | Delivery | Důkaz z GitHubu po publikování této změny |
