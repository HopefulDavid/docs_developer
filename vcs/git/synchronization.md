...
Upstream je sledovaná vzdálená větev pro porovnání stavu a výchozí synchronizaci; první push ji běžně nastavuje přes `-u`.
...
Tři tečky v tomto Git výrazu skutečně napíšeš; porovnávají commity, které jsou jen na jedné z obou stran.
...
`pull origin main` spuštěný na pracovní větvi začleňuje `main` do této pracovní větve; nepřepíná za tebe větev.
...
Tyto dvě varianty nespouštěj po sobě jako jeden postup; jejich rozdíl a řešení konfliktů popisuje [slučování](merging.md).
...
Další odeslání může používat explicitní `git push origin feature/hledani`; na `main` názvy přizpůsob skutečné větvi.
...
Po vlastním amend nebo rebase mohou mít logicky stejné změny jiná ID; publikování přepsané vlastní větve řeší [opravy historie](history/fix-commits.md#publikování-přepsané-vlastní-větve).
...
Názvy nahraď svou větví; samotný stav „up to date“ bez čerstvého fetch může vycházet ze staršího místního záznamu serveru.
