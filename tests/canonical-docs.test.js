...
test('publikování udržuje gh-pages jako jediný kořenový commit', () => {
  const workflow = fs
    .readFileSync(path.join(root, '.github', 'workflows', 'main.yml'), 'utf8')
    .replace(/\r\n/g, '\n');
  const delivery = fs.readFileSync(path.join(docsRoot, 'delivery', 'ci-cd.md'), 'utf8');
